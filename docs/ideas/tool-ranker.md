...
3. **Run a Jev rerank on the same set as a ceiling check.** One experiment file in this harness, shaped like `experiments/tool-ranking.ts`: embeddings pick the top 50 to 250 into each record, a per-record Choice over them (up to 255 options) reorders them, `derive` computes the rank of the expected tool. The result says how much headroom ranking has at all. If Jev barely beats the enriched hybrid, stop; if it beats it clearly, that is the target a local reranker has to approach.
