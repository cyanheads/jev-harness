...
- **`message_clarity` was the most precise signal.** Its "Opaque" level picked out 27 groups, and on reading them nearly all were real: raw HTML error pages passed through as the message, bare status-code passthroughs, and "connection closed". Several matched issues already filed from hand triage, which is the check that the signal is real.
- **Severity needed a different gate.** The top level's confidence averaged 0.55 on a four-level Score, so "score at least 2 and confidence at least 0.6" escalated 1% of rows. Summing the probability on the top two levels and requiring 0.8 escalated 15%, and sorted by occurrence count that list read as a sensible work queue.
