...
const flagsSchema = z.object({
  ...
  bins: z.coerce.number().int().positive(),
  provider: z.enum(['openrouter', 'typesafe'] satisfies Provider[]).optional(),
});
...
async function main(): Promise<void> {
  ...
  const flags = flagsSchema.parse(values);

  /** A dry run builds the same client the real call would, minus the key. */
  const makeClient = (dryRun = false): JevClient =>
    new JevClient({
      ...(flags.provider !== undefined && { provider: flags.provider }),
      ...(values.model !== undefined && { model: values.model }),
      ...(dryRun && { apiKey: 'dry-run' }),
    });
  ...
  switch (command) {
    ...
    case 'calibrate': {
      ...
      const rows = (await readJsonl(values.rows)) as RunRow[];
      const truth = new Map(
        (await readJsonl(values.truth)).map((line) => {
          const { id, truth } = truthLineSchema.parse(line);
          return [id, truth];
        }),
      );
      console.log(renderCalibration(calibrate(rows, truth, flags.bins)));
      break;
    }
    ...
    case 'stability': {
      ...
      const runs = await Promise.all(rest.map(async (path) => (await readJsonl(path)) as RunRow[]));
